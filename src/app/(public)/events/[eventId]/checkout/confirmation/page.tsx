...
import { Order, OrderStatus, PaymentStatus } from '@/types/payment.types';
...
export default function ConfirmationPage() {
  ...
  const order: Order | null = data?.orderByNumber || null;
      ...
      <div className="container max-w-2xl mx-auto px-4 py-12">
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="p-6 space-y-6">
            {/* Event Details */}
            <div>
              <h2 className="text-lg font-semibold mb-3">{order.event.name}</h2>
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Calendar className="h-4 w-4" />
                  <span>
                    {format(parseISO(order.event.startDate), 'EEEE, MMMM d, yyyy')}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Clock className="h-4 w-4" />
                  <span>
                    {format(parseISO(order.event.startDate), 'h:mm a')} -{' '}
                    {format(parseISO(order.event.endDate), 'h:mm a')}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="capitalize">
                      {order.payment.paymentMethodBrand} •••• {order.payment.paymentMethodLast4}
                    </span>
            ...
            {isCompleted && (
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <Button asChild className="flex-1">
                  <Link href={`/attendee/events/${order.event.id}`}>
